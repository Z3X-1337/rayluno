...
def test_clear_all_requires_specific_confirmation_within_sixty_seconds() -> None:
    clock = MutableClock()
    service = MemoryService(InMemoryMemoryStore(), clock=clock)
    planner = MemoryCommandPlanner(service, clock=clock)
    service.remember("اسمي زيد", category="identity")
    service.remember("أفضل الردود المختصرة", category="preference")

    requested = planner.plan("احذف كل الذاكره", "احذف كل الذاكرة")

    assert requested is not None
    assert "تأكيد حذف كل الذاكرة" in requested.reply
    ...
    assert "2 عناصر" in confirmed.reply
    assert service.list() == ()


def test_clear_all_confirmation_expires_without_deleting() -> None:
    clock = MutableClock()
    service = MemoryService(InMemoryMemoryStore(), clock=clock)
    planner = MemoryCommandPlanner(service, clock=clock)
    service.remember("اسمي زيد", category="identity")

    planner.plan("احذف كل الذاكره", "احذف كل الذاكرة")
    ...
    assert "لم يُحذف شيء" in expired.reply
    assert len(service.list()) == 1
...
def test_unrelated_conversation_is_not_persisted_implicitly(tmp_path: Path) -> None:
    config = AssistantConfig(
        require_wake_word=False,
        audit_path=None,
        tasks_path=tmp_path / "tasks.sqlite3",
    ...
    runtime = build_runtime(config, effects=DryRunEffects(), audit=MemoryAuditLogger())

    runtime.handle("أنا أفضل الردود المختصرة")

    store = MemoryService(SQLiteMemoryStore(config.memory_path), clock=_clock)
    assert store.list() == ()
